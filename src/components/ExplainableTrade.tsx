  ...
  return (
    <div className="bg-surface border border-border rounded-xl p-4 shadow-card animate-fade-in">
      <div className="flex items-center justify-between mb-4">
        ...
        {onClose && (
          <button onClick={onClose} className="text-muted hover:text-text text-xs font-medium transition-colors">Close</button>
        )}
      </div>
          ...
          {recommendation.contributors.map((c, i) => (
            <div key={i} className="flex items-center gap-2 text-xs">
              ...
              <div className="flex-1 h-1.5 bg-border rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${c.side === 'buy' ? 'bg-success' : c.side === 'sell' ? 'bg-danger' : 'bg-muted'}`}
                  style={{ width: `${c.confidence * 100}%` }}
                />
              </div>
              <span className="tabular-nums w-10 text-right text-muted font-medium">{(c.confidence * 100).toFixed(0)}%</span>
            </div>
          ))}
        </div>
      </Section>
          ...
          {recommendation.atr && (
            <>
              <div className="bg-bg/40 rounded-lg p-2.5 border border-border/40">
                ...
                <div className="font-medium tabular-nums text-text">{recommendation.atr.toFixed(2)}</div>
              </div>
              <div className="bg-bg/40 rounded-lg p-2.5 border border-border/40">
                <div className="text-muted mb-0.5">Volatility</div>
                <div className="font-medium text-text">{recommendation.atr > 0 ? 'Moderate' : 'Low'}</div>
              </div>
            </>
          )}
        </div>
...
function Section({ icon: Icon, title, color, children }: { icon: LucideIcon; title: string; color: string; children: React.ReactNode }) {
  return (
    <div className="mb-4 pb-4 border-b border-border/40 last:border-b-0 last:mb-0 last:pb-0">

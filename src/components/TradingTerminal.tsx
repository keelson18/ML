  ...
  useEffect(() => {
    ...
    for (const ov of overlays) {
      if (ov.type === 'line' && ov.points && ov.points.length > 1) {
        ...
        if (first && last) {
          line.setData([{ time: first, value: ov.price }, { time: last, value: ov.price }]);
        }
        overlaySeriesRef.current.push(line);
      } else if (ov.type === 'markers' && ov.markers) {
        for (const m of ov.markers) {
          allMarkers.push({ time: m.time as Time, position: m.position, color: m.color, shape: m.shape, text: m.text });
        }
      }
    }

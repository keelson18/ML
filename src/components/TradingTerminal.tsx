...
export default function TradingTerminal(props: Props) {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener('resize', handleResize); chart.remove(); chartRef.current = null; candleSeriesRef.current = null; };
  }, [theme]);
  ...
  useEffect(() => {
    ...
    for (const ov of overlays) {
      if (ov.type === 'line' && ov.points && ov.points.length > 1) {
        const line = chart.addLineSeries({ color: ov.color ?? '#6b7280', lineWidth: 1, priceLineVisible: false, lastValueVisible: false });
        line.setData(ov.points.filter((p) => !isNaN(p.value)).map((p) => ({ time: p.time as Time, value: p.value })));
        overlaySeriesRef.current.push(line);
      } else if (ov.type === 'hline' && ov.price != null) {
        const line = chart.addLineSeries({ color: ov.color ?? '#9ca3af', lineWidth: 1, lineStyle: 2, priceLineVisible: false, lastValueVisible: true, title: ov.label });
        const first = candles[0]?.time as Time | undefined;
        const last = candles[candles.length - 1]?.time as Time | undefined;
        if (first && last) line.setData([{ time: first, value: ov.price }, { time: last, value: ov.price }]);
        overlaySeriesRef.current.push(line);
      } else if (ov.type === 'markers' && ov.markers) {
        for (const m of ov.markers) allMarkers.push({ time: m.time as Time, position: m.position, color: m.color, shape: m.shape, text: m.text });
      }
    }
